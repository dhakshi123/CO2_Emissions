...
if user_input == 'Visulization':
    ...
    # Dropdown for Make column
    selected_make = st.selectbox("Select Make", sorted(df["Make"].unique()))
     # Dropdown for Model column
    selected_model = st.selectbox("Select Model", sorted(df["Model"].unique()))
    selected_vehiclass = st.selectbox("Select Vehicle Class", sorted(df["Vehicle Class"].unique()))
    engine_size = st.number_input('Engine Size(L) [please enter your CC]',step=0.1, format="%.1f")
    cylinders = st.number_input('Cylinders', min_value=2, max_value=16, step=1)
    selected_Transmission = st.selectbox("Select Transmission", sorted(df["Transmission"].unique()))    
    selected_FuelType = st.selectbox("Select Fuel Type", sorted(df["Fuel Type"].unique()))
    fuel_consumption = st.slider('Fuel Consumption Comb (L/100 km)', min_value=4.0, max_value=50.0, step=0.1, format="%.1f")
